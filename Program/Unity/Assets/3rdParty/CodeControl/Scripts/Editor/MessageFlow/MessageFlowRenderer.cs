...
namespace CodeControl.Editor
{
	public class MessageFlowRenderer : IWindowRenderer
	{
		...
		private void OnMessageHandleHandler(Type callerType, Type handlerType, Type messageType, string messageName)
		{
			...
			line.SendMessage(messageType, messageType.Name, reversed);
		...
		private void FilterLinesAndActors(Predicate<MessageLine> where)
		{
			...
			{
				if (!messageActors.Contains(line.CallerActor))
				{
					messageActors.Add(line.CallerActor);
				}
				if (!messageActors.Contains(line.HandlerActor))
				{
					messageActors.Add(line.HandlerActor);
				}
				messageLines.Add(line);
			}
